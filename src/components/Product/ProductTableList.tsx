...
import { setSelectedProducts } from '../../features/product/productSlice';
        ...
        <Paper elevation={10} style={{ height: 700, width: '100%', margin: '20px auto' }}>
            <DataGrid
                ...
                onSelectionModelChange={(ids: any) => {
                    ...
                    const selectedRowData = products.filter((row: Product) =>
                        selectedIDs.has(row.id.toString())
                    )
                    dispatch(setSelectedProducts(selectedRowData))
                }}
            />
        </Paper>
    )
